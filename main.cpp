...
//   IRBuilder<> irb(&*oldEntry->getFirstInsertionPt()); // generate context
//   info
//                                                       // key for each block
//   Value *visitedArray =
...
//   irb.CreateMemSet(visitedArray, irb.getInt8(0), origBB.size(),
//   (MaybeAlign)0); irb.CreateMemSet(keyArray, irb.getInt8(0), origBB.size() *
//   4, (MaybeAlign)0); int idx = 0; std::vector<unsigned int> key_list;
...
//                              /*isConstant=*/false,
//                              GlobalValue::PrivateLinkage, doms_array,
//                              "doms");
...
//   LoadInst *swValue =
//       new LoadInst(switchVar->getAllocatedType(), switchVar, "cmd",
//       loopBegin);
            ...
            [](PassBuilder &PB)
            {
                PB.registerPipelineParsingCallback(
                       ...
                       ArrayRef<PassBuilder::PipelineElement>)
                    {
                        ...
                        if (Name == "flattening-enhanced")
                        {
                            MPM.addPass(
                                llvm::FlatteningEnhanced(true));  // Use
namespaced class return true;
                        }
                        return false;
                    });
                // Example of registering for a specific optimization pipeline
point PB.registerPipelineStartEPCallback(
                    [](ModulePassManager &MPM, OptimizationLevel Level)
                    {
                        //
MPM.addPass(createModuleToFunctionPassAdaptor(HelloPass()));
                        // // Example
                        MPM.addPass(llvm::FlatteningEnhanced(
                            true));  // Run FlatteningEnhanced early
                    });
            }};
} */