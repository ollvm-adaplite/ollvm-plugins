...
static void linkRuntime(Module &M)
{
    ...
    {
        if (F.getName() != "__tsx_tamper_handler" && F.getName() != "__aead_xchacha20_poly1305_decrypt" && !F.isDeclaration())
        {
                                      ...
                                      FunctionAnalysisManager &AM)
{
    ...
    if (F.getName() == "__tsx_tamper_handler" || F.getName().starts_with("secure_terminate"))
    {
        return PreservedAnalyses::all();
    }
