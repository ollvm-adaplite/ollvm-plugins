/**
 * @file FlatteningEnhancedver1.cpp
 * @brief 
 * @author Lux-QAQ
 * @version 1.0.1
 * @date 2025-12-09
...
PreservedAnalyses FlatteningEnhancedver1::run(Module &M,
                                          ModuleAnalysisManager &AM)
{
    ...
    for (Function &f : M)
    {
        ...
        {
            outs() << "\033[1;32m[FlatteningEnhancedver1] Function: " << f.getName()
                   << "\033[0m\n";  // 打印一下被混淆函数的symbol

            if (&f == updateFunc)
                continue;
            DoFlatteningEnhancedver1(&f, 0, updateFunc);
        }
    }
...
FlatteningEnhancedver1::getBlocks(Function *function,
...
Function *FlatteningEnhancedver1::buildUpdateKeyFunc(Module *m)
{
...
unsigned int
FlatteningEnhancedver1::getUniqueNumber(std::vector<unsigned int> *rand_list)
{
...
void FlatteningEnhancedver1::DoFlatteningEnhancedver1(Function *f, int seed,
                                              Function *updateFunc)
{
    srand(seed);
    ...
    /*  if (verifyFunction(*f, &errs())) {
     errs() << "FlatteningEnhancedver1: Function verification failed for "
            << f->getName() << "\n";
     f->dump();
     exit(1);
   } */
 ...
 * @return FunctionPass*
 */
FlatteningEnhancedver1 *llvm::createFlatteningEnhancedver1(bool flag)
{
    return new FlatteningEnhancedver1(flag);
}
