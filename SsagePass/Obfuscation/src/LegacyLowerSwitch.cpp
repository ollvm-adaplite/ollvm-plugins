                           ...
                           const std::vector<IntRange> &UnreachableRanges)
{
    ...
    BasicBlock *NewNode = BasicBlock::Create(
            Val->getContext(), "NodeBlock", F, OrigBlock->getNextNode());  // 立即插入

    IRBuilder<> B(NewNode);  // 构建器自动在块末尾插入
    Value *Cmp =
            B.CreateICmpSLT(Val, Pivot.Low, "Pivot");  // i1 Cmp = (Val < Pivot)
    ...
    /*====================== 5. 结束本块，添加跳转 ====================*/
    B.CreateCondBr(Cmp, LBranch, RBranch);
...
{
    Function *F = OrigBlock->getParent();
    // Create NewLeaf and insert it into the function right after OrigBlock
    BasicBlock *NewLeaf = BasicBlock::Create(Val->getContext(), "LeafBlock", F,
                                             OrigBlock->getNextNode());

    IRBuilder<> Builder(NewLeaf);  // Inserts instructions at the end of NewLeaf

    ICmpInst *Comp = nullptr;
    if (Leaf.Low == Leaf.High)
    {
        Comp = cast<ICmpInst>(Builder.CreateICmpEQ(Val, Leaf.Low, "SwitchLeaf"));
    }
    else
    {
        ...
        {
            Comp =
                    cast<ICmpInst>(Builder.CreateICmpSLE(Val, Leaf.High, "SwitchLeaf"));
        }
        else if (Leaf.Low->isZero())
        {
            Comp =
                    cast<ICmpInst>(Builder.CreateICmpULE(Val, Leaf.High, "SwitchLeaf"));
        }
        else
        {
            Constant *NegLo = ConstantExpr::getNeg(Leaf.Low);
            Value *Add = Builder.CreateAdd(Val, NegLo, Val->getName() + ".off");
            Constant *UpperBound = ConstantExpr::getAdd(NegLo, Leaf.High);
            Comp =
                    cast<ICmpInst>(Builder.CreateICmpULE(Add, UpperBound, "SwitchLeaf"));
        }
    }

    // Make the conditional branch...
    BasicBlock *Succ = Leaf.BB;
    Builder.CreateCondBr(Comp, Succ, Default);
                                    ...
                                    SmallPtrSetImpl<BasicBlock *> &DeleteList)
{
    ...
    if ((CurBlock != &F->getEntryBlock() && pred_empty(CurBlock)) || CurBlock->getSinglePredecessor() == CurBlock)
    {
    ...
    BranchInst::Create(
            Default, NewDefault);  // Branch from NewDefault to the original Default
    ...
    SI->eraseFromParent();  // Removes SI from CurBlock
