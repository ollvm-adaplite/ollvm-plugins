...
void FlatteningPass::flatten(Function &F)
{
    ...
    if (entryBB.getTerminator() && !isa<InvokeInst>(entryBB.getTerminator()) && !isa<ResumeInst>(entryBB.getTerminator()) && !isa<CatchSwitchInst>(entryBB.getTerminator()) && !isa<CatchReturnInst>(entryBB.getTerminator()) && !isa<CleanupReturnInst>(entryBB.getTerminator()))
    {
    ...
    for (BasicBlock *BB : origBB)
    {
        ...
        else if (BB->getTerminator()->getNumSuccessors() == 1)
        {
            ...
            if (ehRelatedBlocks.count(sucBB))
            {  // 如果后继是EH块，则不修改此跳转
        ...
        else if (BB->getTerminator()->getNumSuccessors() == 2)
        {
            ...
            if (BranchInst *br = dyn_cast<BranchInst>(term))
            {
                if (br->isConditional())
                {
                    ...
                    if (ehRelatedBlocks.count(succTrue) || ehRelatedBlocks.count(succFalse))
                    {  // 如果任一后继是EH块
