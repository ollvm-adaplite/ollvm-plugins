...
static cl::opt<int> randomStrSource(
    "mmfw_randstrsource",
    cl::desc(
        "Choose the length of the random string used in FunctionWrapper"),
    cl::value_desc("Random String Source"), cl::init(1), cl::Optional);

PreservedAnalyses FunctionWrapperPass::run(Module &M,
                                           ModuleAnalysisManager &AM) {
  vector<CallSite *> callsites;
  for (Module::iterator iter = M.begin(); iter != M.end(); iter++) {
    Function &F = *iter; // 迭代每个函数
    if (toObfuscate(flag, &F, "funwra")) {
      outs() << "\033[1;32m[FunctionWrapper] Function : " << F.getName()
             << "\033[0m\n"; // 打印一下被混淆函数的symbol
...
CallSite *FunctionWrapperPass::HandleCallSite(CallSite *CS) {
  ...
  if (calledFunction == nullptr ||
      (!isa<ConstantExpr>(calledFunction) && !isa<Function>(calledFunction)) ||
  ...
  // GlobalValue::LinkageTypes::InternalLinkage, "O0ooOO0o0OO0oO",
  // CS->getParent()->getModule()); // 移除Hikari特征 std::string randstring =
  // rand_str((rand() % (20 - 5 + 1)) + 5); // 生成 长度随机 的ooOO00
  //std::string randstring = rand_str(20); // 生成 长度固定 的ooOO00

  std::string randstring =rand_str((rand() % (20 - 8 + 1)) + 8, randomStrSource); // 生成 长度随机 的ooOO00
  /* Function *func = Function::Create(
      ft, GlobalValue::LinkageTypes::InternalLinkage,
      Twine("Hack" + randstring + "END"),
      CS->getParent()->getModule()); // 优化为随机字符串 // FIXBUGS */
  ...
  // 取出被调函数的 FunctionType（不再依赖指针拆封）
  FunctionType *calleeFTy = cast<Function>(calledFunction)->getFunctionType();

  // 把函数指针 bitcast 成调用现场需要的指针类型
  Value *calleePtr = ConstantExpr::getBitCast(cast<Function>(calledFunction),
                                              CS->getCalledValue()->getType());

  // 创建调用
  Value *retval =
      IRB.CreateCall(calleeFTy, calleePtr, ArrayRef<Value *>(params));
...
FunctionWrapperPass llvm::createFunctionWrapperwithoutptr(bool flag) {
  return  FunctionWrapperPass(flag);
}