...
#define LOAD32H(x, y)                                                                                                                                      \
    {                                                                                                                                                      \
        (x) = ((uint32_t)((y)[0] & 0xFF) << 24) | ((uint32_t)((y)[1] & 0xFF) << 16) | ((uint32_t)((y)[2] & 0xFF) << 8) | ((uint32_t)((y)[3] & 0xFF) << 0); \
    }

#define LOAD64H(x, y)                                                                                                                                                                                                                                                                                      \
    {                                                                                                                                                                                                                                                                                                      \
        (x) = ((uint64_t)((y)[0] & 0xFF) << 56) | ((uint64_t)((y)[1] & 0xFF) << 48) | ((uint64_t)((y)[2] & 0xFF) << 40) | ((uint64_t)((y)[3] & 0xFF) << 32) | ((uint64_t)((y)[4] & 0xFF) << 24) | ((uint64_t)((y)[5] & 0xFF) << 16) | ((uint64_t)((y)[6] & 0xFF) << 8) | ((uint64_t)((y)[7] & 0xFF) << 0); \
    }
...
#define LOAD32H(x, y)                                                                                                                                      \
    {                                                                                                                                                      \
        (x) = ((uint32_t)((y)[3] & 0xFF) << 24) | ((uint32_t)((y)[2] & 0xFF) << 16) | ((uint32_t)((y)[1] & 0xFF) << 8) | ((uint32_t)((y)[0] & 0xFF) << 0); \
    }

#define LOAD64H(x, y)                                                                                                                                                                                                                                                                                      \
    {                                                                                                                                                                                                                                                                                                      \
        (x) = ((uint64_t)((y)[7] & 0xFF) << 56) | ((uint64_t)((y)[6] & 0xFF) << 48) | ((uint64_t)((y)[5] & 0xFF) << 40) | ((uint64_t)((y)[4] & 0xFF) << 32) | ((uint64_t)((y)[3] & 0xFF) << 24) | ((uint64_t)((y)[2] & 0xFF) << 16) | ((uint64_t)((y)[1] & 0xFF) << 8) | ((uint64_t)((y)[0] & 0xFF) << 0); \
    }
...
CryptoUtils::CryptoUtils() : idx(0), seeded(false)
{
    // 初始化所有成员为安全状态
    ...
    // seed string 默认构造为空字符串

    // 尝试自动播种
    if (!myprng_seed())
    {
        errs() << "CryptoUtils: Warning - Failed to seed PRNG in constructor\n";
    }
}
...
// bool CryptoUtils::myprng_seed(std::string const &_seed) {
//   unsigned char s[16];
//   unsigned int i = 0;

//   /* We accept a prefix "0x" */
//   if (!(_seed.size() == 32 || _seed.size() == 34)) {
//     errs() << "The AES-CTR PRNG seeding mechanism is expecting a 16-byte
//     value "
...
// 实现拷贝构造函数
CryptoUtils::CryptoUtils(const CryptoUtils &other) : idx(0), seeded(false)
{
    // 初始化所有成员
    memset(key, 0, sizeof(key));
    memset(ks, 0, sizeof(ks));
    ...
    if (other.seeded)
    {
        try
        {
            ...
            memcpy(ks, other.ks, sizeof(ks));
            memcpy(ctr, other.ctr, sizeof(ctr));
            memcpy(pool, other.pool, sizeof(pool));
            idx = other.idx;
            seed = other.seed;  // 这里可能抛出 bad_alloc
            seeded = true;
        }
        catch (const std::bad_alloc &e)
        {
            // 如果字符串复制失败，尝试重新播种
            errs() << "CryptoUtils: Memory allocation failed in copy constructor, "
                      "attempting to reseed Reason: \n"
                   << e.what() << "\n";
            if (!myprng_seed())
            {
                errs() << "CryptoUtils: Warning - Failed to seed PRNG in copy "
                          "constructor after allocation failure\n";
            }
        }
    }
    else
    {
        // 如果源对象未播种，尝试重新播种
        int retry_count = 3;
        while (retry_count > 0 && !myprng_seed())
        {  // 现在会调用增强版的 myprng_seed()
            retry_count--;
            if (retry_count > 0)
            {
                errs() << "CryptoUtils: Retrying PRNG seeding (" << (3 - retry_count)
                       << "/3)\n";
        ...
        if (!seeded)
        {  // seeded 是成员变量，会被 myprng_seed() 设置
            errs() << "CryptoUtils: Warning - Failed to seed PRNG in copy "
                      "constructor after "
                   << (3 - retry_count) << " attempts\n";
        }
    }
}

// 实现赋值操作符
CryptoUtils &CryptoUtils::operator=(const CryptoUtils &other)
{
    if (this != &other)
    {
        if (other.seeded)
        ...
        else
        {
            ...
            if (!myprng_seed())
            {
                errs() << "CryptoUtils: Warning - Failed to seed PRNG in assignment\n";
            }
        }
    }
    return *this;
...
#define debuglog(msg)                                                        \
    llvm::errs().changeColor(llvm::raw_ostream::YELLOW, true)                \
            << "[Debug] " << __FUNCTION__ << ":" << __LINE__ << " - " << msg \
            << "\n",                                                         \
            llvm::errs().resetColor()
bool CryptoUtils::myprng_seed(std::string const &_seed)
{
    debuglog("Starting myprng_seed with parameter: " + _seed);

    errs()
            << "CryptoUtils::myprng_seed(std::string const &_seed) called with seed: "
            << _seed << "\n";

    debuglog("Initializing variables for seed parsing...");
    unsigned char s[16];
    // It's good practice to initialize i.
    // Based on the logic, if _seed.size() == 34, i should be 2, otherwise 0.
    unsigned int i = 0;

    debuglog("Checking seed string format and length...");
    debuglog("Seed length: " + std::to_string(_seed.size()));

    /* We accept a prefix "0x" */
    if (!(_seed.size() == 32 || (_seed.size() == 34 && _seed.rfind("0x", 0) == 0)))
    {
        debuglog("Seed format validation failed");
        debuglog("Expected: 32 chars (hex) or 34 chars with '0x' prefix");
        debuglog("Actual: " + std::to_string(_seed.size()) + " chars");

        errs() << "The AES-CTR PRNG seeding mechanism is expecting a 16-byte value "
                  "expressed in hexadecimal (32 chars), optionally prefixed with "
                  "\"0x\" (34 chars).\n";
        // Ensure seeded remains false or handle error more robustly
        debuglog("Setting seeded flag to false due to format error");
        this->seeded = false;  // Explicitly set seeded to false on error
        debuglog("Returning false from myprng_seed due to format validation failure");
        return false;
    }

    debuglog("Seed format validation passed");
    debuglog("Storing original seed string...");
    this->seed = _seed;  // Store the original string seed
    debuglog("Original seed string stored successfully");

    debuglog("Checking if seed has '0x' prefix...");
    if (_seed.size() == 34)
    {  // && _seed.rfind("0x", 0) == 0 is checked above
        debuglog("Seed has '0x' prefix, setting start index to 2");
        // Assuming that the two first characters are "0x"
        i = 2;
    }
    else
    {
        debuglog("Seed has no prefix, starting from index 0");
    }

    // Initialize s to prevent using uninitialized values if loop doesn't fill it
    // completely
    debuglog("Initializing byte array to zero...");
    memset(s, 0, sizeof(s));
    debuglog("Byte array initialized");

    unsigned int s_idx = 0;
    debuglog("Starting hex string parsing loop...");
    debuglog("Parse start index: " + std::to_string(i));
    debuglog("Expected iterations: " + std::to_string((_seed.size() - i) / 2));

    for (/* i is already set */; i + 1 < _seed.size() && s_idx < 16;
         i += 2, s_idx++)
    {
        debuglog("Processing hex pair at index " + std::to_string(i) + " (byte " + std::to_string(s_idx) + "/16)");

        std::string byte_str = _seed.substr(i, 2);
        debuglog("Extracted hex string: '" + byte_str + "'");

        char *endptr;
        debuglog("Calling strtol for hex conversion...");
        long val = strtol(byte_str.c_str(), &endptr, 16);
        debuglog("strtol result: " + std::to_string(val));

        if (*endptr != '\0')
        {
            debuglog("Invalid hex character detected in: '" + byte_str + "'");
            debuglog("Endptr points to: '" + std::string(1, *endptr) + "'");

            errs() << "Invalid hex character in seed: " << byte_str << "\n";
            debuglog("Setting seeded flag to false due to invalid hex");
            this->seeded = false;  // Mark as not seeded on error
            debuglog("Returning false from myprng_seed due to hex validation failure");
            return false;
        }

        debuglog("Hex conversion successful, storing byte value: " + std::to_string(val));
        s[s_idx] = (unsigned char)val;
        debuglog("Byte " + std::to_string(s_idx) + " stored successfully");
    }

    debuglog("Hex parsing loop completed");
    debuglog("Final s_idx value: " + std::to_string(s_idx));
    debuglog("Expected s_idx: 16");

    if (s_idx != 16)
    {
        debuglog("Byte count validation failed");
        debuglog("Expected 16 bytes, got: " + std::to_string(s_idx));

        errs() << "Seed string did not yield 16 bytes of data. Got: " << s_idx
               << "\n";
        debuglog("Setting seeded flag to false due to byte count mismatch");
        this->seeded = false;  // Mark as not seeded on error
        debuglog("Returning false from myprng_seed due to byte count validation failure");
        return false;
    }

    debuglog("Byte count validation passed, copying to key array...");
    // _seed is defined to be the
    // key initial value
    memcpy(key, s, 16);
    debuglog("Key array populated successfully");

    DEBUG_WITH_TYPE("cryptoutils", dbgs()
                                           << "CPNRG seeded with " << _seed << "\n");

    debuglog("Initializing counter to zero...");
    // ctr is initialized to all-zeroes
    memset(ctr, 0, 16);
    debuglog("Counter initialized successfully");

    debuglog("Computing AES key schedule...");
    // Once the seed is there, we compute the
    // AES128 key-schedule
    aes_compute_ks(ks, key);
    debuglog("AES key schedule computed successfully");

    debuglog("Populating random pool...");
    // We are now ready to fill the pool with
    // cryptographically secure pseudo-random
    // values.
    populate_pool();  // Call populate_pool() here
    debuglog("Random pool populated successfully");

    debuglog("Setting seeded flag to true...");
    seeded = true;  // Set seeded to true only after successful seeding and pool
                    // population
    debuglog("Seeded flag set to true");

    debuglog("myprng_seed with parameter completed successfully");
    debuglog("Returning true from myprng_seed");
    return true;
}
...
bool CryptoUtils::myprng_seed()
{
    debuglog("Attempting to seed PRNG...");
    bool system_source_succeeded = false;
    std::string dev_path;  // 用于记录尝试的设备路径

#if defined(__linux__)
    dev_path = "/dev/urandom";
    debuglog("Setting dev_path to: " + dev_path);
    debuglog("Attempting to open " + dev_path + " for reading...");

    std::ifstream devrandom(dev_path, std::ios::in | std::ios::binary);
    debuglog("ifstream constructor completed");

    // 检查文件是否成功打开
    debuglog("Checking if " + dev_path + " is open...");
    bool is_open = devrandom.is_open();
    debuglog("is_open() result: " + std::string(is_open ? "true" : "false"));

    if (!is_open)
    {
        debuglog("Failed to open file, getting errno...");
        int current_errno = errno;
        debuglog("errno value: " + std::to_string(current_errno));
        errs() << "CryptoUtils: Error - Failed to open " << dev_path
               << ". errno: " << current_errno << " (" << strerror(current_errno)
               << ")\n";
    }
    else
    {
        debuglog("File opened successfully, checking stream state...");
        bool is_fail = devrandom.fail();
        debuglog("fail() result: " + std::string(is_fail ? "true" : "false"));

        if (is_fail)
        {
            debuglog("Stream is in fail state, getting error details...");
            int current_errno = errno;
            debuglog("errno after fail check: " + std::to_string(current_errno));
            bool is_good = devrandom.good();
            bool is_bad = devrandom.bad();
            bool is_eof = devrandom.eof();
            debuglog(
                    "Stream states - good: " + std::string(is_good ? "true" : "false") + ", bad: " + std::string(is_bad ? "true" : "false") + ", fail: " + std::string(is_fail ? "true" : "false") + ", eof: " + std::string(is_eof ? "true" : "false"));

            errs()
                    << "CryptoUtils: Error - Stream state error after attempting to open "
                    << dev_path << ". errno: " << current_errno << " ("
                    << strerror(current_errno) << ")"
                    << ". Stream state (good/bad/fail/eof): " << is_good << "/" << is_bad
                    << "/" << is_fail << "/" << is_eof << "\n";

            debuglog("Attempting to close stream...");
            devrandom.close();
            debuglog("Stream closed");
        }
        else
        {
            debuglog("Stream is good, attempting to read 16 bytes...");
            devrandom.read(this->key, 16);
            debuglog("Read operation completed");

            debuglog("Getting byte count from gcount()...");
            std::streamsize bytes_read = devrandom.gcount();
            debuglog("Bytes read: " + std::to_string(bytes_read));

            if (bytes_read == 16)
            {
                debuglog("Successfully read 16 bytes");
                system_source_succeeded = true;
                debuglog("Setting system_source_succeeded to true");
                DEBUG_WITH_TYPE("cryptoutils",
                                dbgs() << "CryptoUtils: Seeded successfully from "
                                       << dev_path << "\n");
            }
            else
            {
                debuglog("Read count mismatch, getting stream state...");
                int current_errno = errno;
                bool is_good = devrandom.good();
                bool is_bad = devrandom.bad();
                bool is_fail = devrandom.fail();
                bool is_eof = devrandom.eof();
                debuglog("Post-read errno: " + std::to_string(current_errno));
                debuglog("Post-read stream states - good: " + std::string(is_good ? "true" : "false") + ", bad: " + std::string(is_bad ? "true" : "false") + ", fail: " + std::string(is_fail ? "true" : "false") + ", eof: " + std::string(is_eof ? "true" : "false"));

                errs() << "CryptoUtils: Warning - Failed to read 16 bytes from "
                       << dev_path << " (got " << bytes_read
                       << "). Stream state (good/bad/fail/eof): " << is_good << "/"
                       << is_bad << "/" << is_fail << "/" << is_eof
                       << ". errno after read: " << current_errno << " ("
                       << strerror(current_errno) << ")\n";
            }
            debuglog("Attempting to close stream...");
            devrandom.close();
...
#elif defined(_WIN64) || defined(_WIN32)
    dev_path = "CryptGenRandom";
    debuglog("Setting dev_path to: " + dev_path);
    debuglog("Creating WinDevRandom instance...");

    WinDevRandom devrandom;  // Constructor attempts CryptAcquireContext
    debuglog("WinDevRandom constructor completed");

    debuglog("Checking if WinDevRandom is good...");
    bool is_good = devrandom.good();
    debuglog("good() result: " + std::string(is_good ? "true" : "false"));

    if (is_good)
    {
        debuglog("WinDevRandom is good, attempting to read 16 bytes...");
        size_t bytes_read = devrandom.read(this->key, 16);
        debuglog("Read operation completed, bytes_read: " + std::to_string(bytes_read));

        if (bytes_read == 16)
        {
            debuglog("Successfully read 16 bytes from WinDevRandom");
            system_source_succeeded = true;
            debuglog("Setting system_source_succeeded to true");
            DEBUG_WITH_TYPE("cryptoutils",
                            dbgs() << "CryptoUtils: Seeded successfully from "
        ...
        {
            debuglog("WinDevRandom read failed");
            // WinDevRandom::read 内部应该已经打印了 CryptGenRandom 的错误
            errs() << "CryptoUtils: Warning - Failed to read 16 bytes using "
                   << dev_path << " (see previous GetLastError() output).\n";
        }
        debuglog("WinDevRandom will close on destruction");
        // WinDevRandom closes on destruction
    }
    else
    {
        debuglog("WinDevRandom is not good, initialization failed");
        // WinDevRandom constructor 内部应该已经打印了 CryptAcquireContext 的错误
        errs() << "CryptoUtils: Warning - Failed to initialize " << dev_path
               << " for seeding (see previous GetLastError() output).\n";
    }
#else
    // Fallback for other/unknown systems, try /dev/random as a last resort for
    // system entropy
    dev_path = "/dev/random";
    debuglog("Setting dev_path to: " + dev_path + " (fallback for unknown system)");
    debuglog("Attempting to open " + dev_path + " for reading...");
    ...
    debuglog("Checking if system_source_succeeded: " + std::string(system_source_succeeded ? "true" : "false"));
    if (!system_source_succeeded)
    {
        debuglog("System entropy source failed, attempting fallback...");
        errs() << "CryptoUtils: Warning - System entropy source ("
               << (dev_path.empty() ? "N/A" : dev_path)
               << ") failed. Attempting fallback using std::random_device.\n";

        debuglog("Entering try block for std::random_device fallback...");
        try
        {
            debuglog("Creating std::random_device instance...");
            std::random_device rd;
            debuglog("std::random_device created successfully");

            debuglog("Getting entropy from random_device...");
            double entropy = rd.entropy();
            debuglog("Entropy value: " + std::to_string(entropy));
            DEBUG_WITH_TYPE("cryptoutils",
                            dbgs() << "CryptoUtils: std::random_device entropy: "
                                   << entropy << "\n");
            if (entropy == 0)
            {
                debuglog("Warning: entropy is 0");
                errs() << "CryptoUtils: Note - std::random_device entropy is 0, may be "
                          "using a pseudo-random engine as fallback source.\n";
            }

            debuglog("Getting random value from random_device...");
            unsigned long seed_val_rd = rd();  // Potentially throws if rd fails
            debuglog("Random device value: " + std::to_string(seed_val_rd));

            debuglog("Getting current time for additional seed component...");
            unsigned long seed_val_time =
                    std::chrono::high_resolution_clock::now().time_since_epoch().count();
            debuglog("Time value: " + std::to_string(seed_val_time));

            DEBUG_WITH_TYPE(
                    "cryptoutils",
                    dbgs() << "CryptoUtils: Fallback seed components: random_device_val="
                           << seed_val_rd << ", time_val=" << seed_val_time << "\n");

            debuglog("Creating seed_seq with values...");
            std::seed_seq sseq{seed_val_rd, seed_val_time};
            debuglog("seed_seq created successfully");

            debuglog("Creating mt19937_64 generator with seed_seq...");
            ...
            debuglog("Key array filled successfully");

            DEBUG_WITH_TYPE("cryptoutils",
                            dbgs() << "CryptoUtils: Successfully seeded using "
                                      "std::random_device fallback.\n");
            system_source_succeeded = true;  // Mark as succeeded via fallback
            debuglog(
                    "Setting system_source_succeeded to true after fallback success");
        }
        catch (const std::exception &e)
        {
            debuglog("Exception caught in fallback seeding");
            //debuglog("Exception type: " + std::string(typeid(e).name()));
            debuglog("Exception message: " + std::string(e.what()));
            errs() << "CryptoUtils: CRITICAL - std::random_device fallback failed: "
                   << e.what() << ". Type: "  //<< typeid(e).name()
                   << ". PRNG will remain unseeded.\n";
            debuglog("Setting seeded to false due to exception");
            this->seeded = false;
            debuglog("Returning false due to fallback failure");
            return false;
        }
        debuglog("Exited try-catch block for fallback seeding");
    }

    debuglog("Final check: system_source_succeeded = " + std::string(system_source_succeeded ? "true" : "false"));
    if (system_source_succeeded)
    {
        debuglog("Seeding succeeded, proceeding with post-processing...");

        debuglog("Clearing existing seed string...");
        this->seed.clear();
        debuglog("Seed string cleared");

        debuglog("Converting key bytes to hexadecimal string...");
        char hex_byte[3];
        for (int i_key = 0; i_key < 16; ++i_key)
        {
            debuglog("Converting byte " + std::to_string(i_key) + " to hex...");
            int snprintf_result =
        ...
        debuglog("Hex conversion completed");

        DEBUG_WITH_TYPE("cryptoutils",
                        dbgs() << "CryptoUtils: Internal string seed set to: "
                               << this->seed << "\n");

        debuglog("Initializing counter to zero...");
        std::memset(this->ctr, 0, 16);
        debuglog("Counter initialized");

        debuglog("Computing AES key schedule...");
        aes_compute_ks(this->ks, this->key);
        debuglog("Key schedule computed");

        debuglog("Populating random pool...");
        populate_pool();
        debuglog("Pool populated");

        debuglog("Setting seeded flag to true...");
        this->seeded = true;
        debuglog("Seeded flag set");

        DEBUG_WITH_TYPE(
                "cryptoutils",
                dbgs() << "CryptoUtils: PRNG fully seeded and pool populated.\n");
        debuglog("Returning true - seeding successful");
        return true;
    }

    debuglog("All seeding attempts failed");
    errs() << "CryptoUtils: CRITICAL - All seeding attempts (system and "
              "fallback) failed. PRNG is unseeded.\n";
    debuglog("Setting seeded to false");
    this->seeded = false;
    debuglog("Returning false - seeding failed");
    return false;
}
...
void CryptoUtils::get_bytes(char *buffer, const int len)
{
    ...
    if (len > 0)
    {
        // If the PRNG is not seeded, try to seed it.
        // myprng_seed() now calls populate_pool() internally if successful.
        if (!seeded)
        {
            if (!myprng_seed())
            {
                // Seeding failed, cannot provide bytes.
                // This case should be handled, e.g., by an error or ensuring buffer is
                // not used. For now, we might return or fill with a known pattern,
                // though ideally, seeding should not fail.
                errs() << "FATAL: CryptoUtils::get_bytes - PRNG seeding failed. Cannot "
                          "generate random bytes.\n";
                // Depending on desired behavior, either abort, throw, or fill buffer
                // with non-random data. For safety, let's fill with zeros if seeding
                // fails and we must return.
                memset(buffer, 0, len);  // Or handle error more drastically
                return;
            }
            // If myprng_seed() was successful, it already called populate_pool().
        }

        do
        {
            // Check if the pool needs repopulating *before* trying to read from it.
            // This also covers the initial case where idx is 0 and pool might be
            // empty if not populated by constructor/initial myprng_seed call. With
            // the fix to myprng_seed, the pool should be populated if seeded is true.
            if (idx >= CryptoUtils_POOL_SIZE)
            {                     // Pool is exhausted
                populate_pool();  // Refill the pool
            }

            if (idx + (len - sofar) >= CryptoUtils_POOL_SIZE)
            {
                // We don't have enough bytes ready in the pool for the *remaining*
                // request, so let's use the available ones and repopulate for the next
                // iteration if needed.
                available = CryptoUtils_POOL_SIZE - idx;
                memcpy(buffer + sofar, pool + idx, available);
                idx += available;  // Mark these bytes as used
                sofar += available;
                // If sofar < len, the loop will continue, and populate_pool() will be
                // called at the start of the next iteration if idx >= POOL_SIZE
            }
            else
            {
                memcpy(buffer + sofar, pool + idx, len - sofar);
        ...
        } while (sofar < len);  // Loop until all requested bytes are copied
    }
}
...
void CryptoUtils::aes_compute_ks(uint32_t *ks, const char *k)
{
    ...
    {
        tmp = p[3];
        tmp = ((AES_TE4_3(BYTE(tmp, 2))) ^ (AES_TE4_2(BYTE(tmp, 1))) ^ (AES_TE4_1(BYTE(tmp, 0))) ^ (AES_TE4_0(BYTE(tmp, 3))));
...
void CryptoUtils::aes_encrypt(char *out, const char *in, const uint32_t *ks)
{
    ...
    while (1)
    {
        ...
        tmp0 = AES_TE0(BYTE(state0, 3)) ^ AES_TE1(BYTE(state1, 2)) ^ AES_TE2(BYTE(state2, 1)) ^ AES_TE3(BYTE(state3, 0)) ^ ks[r + 0];

        tmp1 = AES_TE0(BYTE(state1, 3)) ^ AES_TE1(BYTE(state2, 2)) ^ AES_TE2(BYTE(state3, 1)) ^ AES_TE3(BYTE(state0, 0)) ^ ks[r + 1];

        tmp2 = AES_TE0(BYTE(state2, 3)) ^ AES_TE1(BYTE(state3, 2)) ^ AES_TE2(BYTE(state0, 1)) ^ AES_TE3(BYTE(state1, 0)) ^ ks[r + 2];

        tmp3 = AES_TE0(BYTE(state3, 3)) ^ AES_TE1(BYTE(state0, 2)) ^ AES_TE2(BYTE(state1, 1)) ^ AES_TE3(BYTE(state2, 0)) ^ ks[r + 3];
    ...
    r += 4;
    state0 = (AES_TE4_3(BYTE(tmp0, 3))) ^ (AES_TE4_2(BYTE(tmp1, 2))) ^ (AES_TE4_1(BYTE(tmp2, 1))) ^ (AES_TE4_0(BYTE(tmp3, 0))) ^ ks[r + 0];

    state1 = (AES_TE4_3(BYTE(tmp1, 3))) ^ (AES_TE4_2(BYTE(tmp2, 2))) ^ (AES_TE4_1(BYTE(tmp3, 1))) ^ (AES_TE4_0(BYTE(tmp0, 0))) ^ ks[r + 1];

    state2 = (AES_TE4_3(BYTE(tmp2, 3))) ^ (AES_TE4_2(BYTE(tmp3, 2))) ^ (AES_TE4_1(BYTE(tmp0, 1))) ^ (AES_TE4_0(BYTE(tmp1, 0))) ^ ks[r + 2];

    state3 = (AES_TE4_3(BYTE(tmp3, 3))) ^ (AES_TE4_2(BYTE(tmp0, 2))) ^ (AES_TE4_1(BYTE(tmp1, 1))) ^ (AES_TE4_0(BYTE(tmp2, 0))) ^ ks[r + 3];
