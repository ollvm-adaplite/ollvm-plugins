...
static cl::opt<int> SplitNum("mmsplit_num", cl::init(3), cl::desc("Split <split_num> time(s) each BB")); 
// 貌似NEW PM暂时不支持这种传递
 ...
 * @return FunctionPass
 */
SplitBasicBlockPass llvm::createSplitBasicBlockWithOutPtr(bool flag){
    return  SplitBasicBlockPass(flag);
}