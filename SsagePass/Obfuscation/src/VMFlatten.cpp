...
#define debugPrint(msg)                                                 \
    do                                                                  \
    {                                                                   \
        outs() << "\033[1;33m[VMFlattening:" << __LINE__ << "] " << msg \
               << "\033[0m\n";                                          \
                                                                        \
    } while (0);
#else
// 关闭调试模式时，使用 do-while(0) 包装并消费参数
#define debugPrint(msg) \
    do                  \
    {                   \
        (void)(msg);    \
...
// 定义一些需要跳过的函数的可能的集合
std::vector<string> O2AvoidFunctions = {
        "_Function_handler", "_M_manager", "_Function_", "_Hash",
        "_Compiler", "__detail", "basic_string"};

std::vector<string> O1AvoidFunctions = {"_Function_handler", "_M_manager",
                                        "_Hash"};

std::vector<string> O3AvoidFunctions = {
        "std::", "__cxa_", "_Unwind_", "_ZSt",
        "_ZN3std", "_ZNSt", "_Function_handler", "_M_manager",
        "_Function_", "_Hash", "_Compiler", "__detail",
        "basic_string", "__invoke", "function", "lambda",
        "_M_invoke"};

std::vector<string> O0AvoidFunctions = {};

auto choosetheAvoidFunctions = [](int opt_level) -> std::vector<string> &
{
    debugPrint("Choosing avoid functions for optimization level: " + std::to_string(opt_level));
    switch (opt_level)
    {
        case 0:
            return O0AvoidFunctions;  // O0级别的跳过函数列表
        case 1:
            return O1AvoidFunctions;  // 默认的跳过函数列表
        case 2:
            return O2AvoidFunctions;  // O2级别的跳过函数列表
        case 3:
            return O3AvoidFunctions;  // O3级别的跳过函数列表
        default:
            return O1AvoidFunctions;  // 默认处理
    }
};
...
PreservedAnalyses VMFlattenPass::run(Function &F, FunctionAnalysisManager &AM)
{
    ...
    std::vector<string> &avoidFunctions = choosetheAvoidFunctions(opt_level);
...
inline std::set<llvm::BasicBlock *> getEHBlocks(llvm::Function *F)
{
    ...
    {
        if (llvm::isa<llvm::InvokeInst>(BB.getTerminator()) || BB.isLandingPad() || llvm::isa<llvm::ResumeInst>(BB.getTerminator()))
        {
            EH.insert(&BB);
            if (BB.isLandingPad())
                WL.push(&BB);
    ...
    while (!WL.empty())
    {
        ...
        WL.pop();
        if (llvm::isa<llvm::ReturnInst>(BB->getTerminator()) || llvm::isa<llvm::ResumeInst>(BB->getTerminator()) || llvm::isa<llvm::UnreachableInst>(BB->getTerminator()))
            continue;
...
std::set<BasicBlock *> getEHBlocks2(Function *F)
{
    ...
    for (BasicBlock &BB : *F)
    {
        if (isa<InvokeInst>(BB.getTerminator()) || BB.isLandingPad() || isa<ResumeInst>(BB.getTerminator()))
        {
            EH.insert(&BB);
            if (BB.isLandingPad())
                WL.push(&BB);
    ...
    while (!WL.empty())
    {
        ...
        WL.pop();
        if (isa<ReturnInst>(BB->getTerminator()) || isa<ResumeInst>(BB->getTerminator()) || isa<UnreachableInst>(BB->getTerminator()))
            continue;
...
void VMFlattenPass::validateVMInstructions(std::vector<VMInst *> *all_inst)
{
    for (size_t i = 0; i < all_inst->size(); i++)
    {
        VMInst *inst = all_inst->at(i);

        // 验证跳转目标是否有效
        if (inst->type == JMP_BORING || inst->type == JMP_SELECT)
        {
            if (inst->op1 >= all_inst->size() * 3)
            {
                errs() << "[VM] Warning: Invalid jump target op1=" << inst->op1
                       << " at instruction " << i << "\n";
                inst->op1 = 0;  // 重置为安全值
            }
            if (inst->type == JMP_SELECT && inst->op2 >= all_inst->size() * 3)
            {
                errs() << "[VM] Warning: Invalid jump target op2=" << inst->op2
                       << " at instruction " << i << "\n";
                inst->op2 = 0;  // 重置为安全值
            }
        }
...
void VMFlattenPass::DoFlatten(Function *f, int seed)
{
    ...
    for (BasicBlock &BB : *f)
    {
        ...
        {
            if (isa<InvokeInst>(&I) || isa<LandingPadInst>(&I) || isa<ResumeInst>(&I) || isa<CatchSwitchInst>(&I) || isa<CatchReturnInst>(&I) || isa<CleanupReturnInst>(&I))
            {
                isEHBlock = true;
                break;
            }
            ...
            if (CallInst *CI = dyn_cast<CallInst>(&I))
            {
                ...
                if (CalledF)
                {
                    ...
                    if (name.starts_with("__cxa_") || name.starts_with("_Unwind_") || name.starts_with("_ZTI") ||  // typeinfo
                        name.starts_with("_ZTV") ||                                                                // vtable
                        name.contains("exception") || name.contains("terminate") || name.contains("catch") || name == "__clang_call_terminate")
                    {
                        isEHBlock = true;
                        break;
                    }
    ...
    debugPrint("Original blocks: " + std::to_string(origBB.size() + ehRelatedBlocks.size()) + ", EH blocks: " + std::to_string(ehRelatedBlocks.size()) + ", Flattening blocks: " + std::to_string(origBB.size()));

    if (origBB.size() <= 1)
    {
        // errs() << "[VM] Too few blocks to flatten, skipping.\n";
        debugPrint("Too few blocks to flatten, skipping.");
        return;
    }
    ...
    if ((firstBr != nullptr && firstBr->isConditional()) || oldEntry->getTerminator()->getNumSuccessors() > 2)
    {
    ...
    validateVMInstructions(&all_inst);
    dump_inst(&all_inst);
    ...
    // 防越界的安全数组访问
    // =================================================================
    Value *arraySize = ConstantInt::get(Type::getInt32Ty(Ctx), opcodes.size());
    Value *rawPC = IRB.CreateLoad(Type::getInt32Ty(Ctx), vm_pc);
    Value *safePC = IRB.CreateURem(rawPC, arraySize);

    Value *op1_offset = IRB.CreateURem(
            IRB.CreateAdd(rawPC, ConstantInt::get(Type::getInt32Ty(Ctx), 1)),
            arraySize);
    Value *op2_offset = IRB.CreateURem(
            IRB.CreateAdd(rawPC, ConstantInt::get(Type::getInt32Ty(Ctx), 2)),
            arraySize);

    Value *optypeGEP =
            IRB.CreateGEP(oparr_var->getValueType(), oparr_var, {zero, safePC});
    ...
    // 安全的PC更新
    IRB.CreateStore(
            IRB.CreateURem(IRB.CreateAdd(IRB.CreateLoad(Type::getInt32Ty(Ctx), vm_pc),
    ...
    for (BasicBlock *block : origBB)
    {
        ...
        {
            if (isa<InvokeInst>(&I) || isa<LandingPadInst>(&I) || isa<ResumeInst>(&I) || isa<CatchReturnInst>(&I) || isa<CatchSwitchInst>(&I) || isa<CleanupReturnInst>(&I))
            {
            ...
            if (CallInst *CI = dyn_cast<CallInst>(&I))
            {
                ...
                if (F && (F->getName().starts_with("__cxa_") || F->getName().starts_with("_Unwind_") || F->getName().contains("exception")))
                {
                    isExceptionRelated = true;
                    break;
                }
    ...
    debugPrint("All blocks processed, now handling jumps.");

    // =================================================================
    // 修复jmp_boring和jmp_select分支的PC设置，使用模运算防越界
    // =================================================================
    IRB.SetInsertPoint(jmp_boring);
    Value *safePCBoring = IRB.CreateURem(op1, arraySize);
    IRB.CreateStore(safePCBoring, vm_pc);
    ...
    Value *safePCTrue = IRB.CreateURem(op1, arraySize);
    IRB.CreateStore(safePCTrue, vm_pc);
    IRB.CreateBr(vm_entry);

    IRB.SetInsertPoint(select_false);
    Value *safePCFalse = IRB.CreateURem(op2, arraySize);
    IRB.CreateStore(safePCFalse, vm_pc);
    IRB.CreateBr(vm_entry);
    ...
    do
    {
        ...
        if ((sameRegsAsBefore && iterations > 0) || ++iterations >= MAX_ITERATIONS)
        {
            /* outs()
          ...
          << " iterations, could not demote all registers.\033[0m\n"; */
            ...
            debugPrint("Breaking demoting loop after " + std::to_string(iterations) + " iterations.");
        ...
        debugPrint("Found " + std::to_string(tmpReg.size()) + " registers and " + std::to_string(tmpPhi.size()) + " PHI nodes to demote.");
  ...
  */
        ...
        debugPrint("Demoting PHI nodes to stack, count: " + std::to_string(tmpPhi.size()));
