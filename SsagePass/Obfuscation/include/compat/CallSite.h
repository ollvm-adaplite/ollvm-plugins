...
public:
  ...
  void setCalledFunction(Value *V) {
    ...
    // 获取需要用来校验的函数类型（Pointee type），不依赖 pointer type
    const auto getExpectedCalleeType = [&]() -> FunctionType * {
      if (auto *CB = dyn_cast<CallBase>(getInstruction())) {
        return CB->getFunctionType();
      }
      llvm_unreachable("Instruction is not a CallBase!");
    };

    // 如有校验需求，可以开启下面这段代码：
    /*
    FunctionType *expectedTy = getExpectedCalleeType();
    Type *actualTy = nullptr;

    // 有些 codepath 旧逻辑中可能获取 pointee type，这里改为从调用参数获取
    if (auto *CB = dyn_cast<CallBase>(getInstruction())) {
      actualTy = CB->getFunctionType();
    }
...
bool doesNotCapture(unsigned OpNo) const {
  auto *CB = cast<CallBase>(getInstruction());
  LLVMContext &Ctx = CB->getContext();

  // A. 推荐方式 — 使用 string-string 重载
  Attribute A = Attribute::get(Ctx, "captures", "none");

  // B. 或者使用 enum + integer 方案
  // Attribute A = Attribute::get(Ctx, AttrKind::Captures, 0);

  return dataOperandHasImpliedAttr(OpNo + 1, A);
}


#elif LLVM_VERSION_MAJOR >= 18
  bool doesNotCapture(unsigned OpNo) const {
    // 使用 captures(none) 替代原来的 nocapture
    return dataOperandHasImpliedAttr(OpNo + 1, Attribute::Captures) &&
           getAttributeType(OpNo + 1, Attribute::Captures) == "none";
  }
#else
  bool doesNotCapture(unsigned OpNo) const {
